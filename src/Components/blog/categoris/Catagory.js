...
import { deleteBlogCategory, getAllBlogCategory, searchBlogCategory } from '../../../api/Api';
...
function Category() {
  ...
  const getData = async (page) => {
    if (searchStatus) {
      ...
      try {
        setLoading(true);
        const res = await getAllBlogCategory(page);
  ...
  const handleDelete = async () => {
    try {
      setLoading(true)
      const res = await deleteBlogCategory(itemId);
      setLoading(false)
  ...
  const searchData = async (pageNo) => {
    try {
      setLoading(true);
      const res = await searchBlogCategory({ page: pageNo, val: inpVal });
      setLoading(false);
      setListData(res?.data);
      setTotalCount(res?.totalCount);
        ...
        <div className="card">
          <div className="card-header row gutters-5">
            <div className="col-lg-4 text-center text-md-left">
              <h5 className="mb-md-0 h6">All Blog Categories</h5>
            </div>
              ...
              <tbody>
                {listData && listData?.map((item, i) => {
                  return <tr key={i}>
                    ...
                    <td style={{ display: 'table-cell' }}>{item.category_name}</td>
