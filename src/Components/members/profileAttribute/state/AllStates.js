...
import { deleteState, getStates, searchState } from '../../../../api/Api';
...
function AllStates() {
    ...
    const getData = async (page) => {
        if (searchStatus) {
            ...
            try {
                setLoading(true);
                const res = await getStates(page);
    ...
    const handleDelete = async () => {
        ...
        try {
            setLoading(true)
            const res = await deleteState(itemId);
    ...
    const searchData = async (pageNo) => {
        ...
        try {
            setLoading(true);
            const res = await searchState({ page: pageNo, val: inpVal });
            setLoading(false);
            setListData(res?.data);
            setTotalCount(res?.totalCount);
