...
import { deleteCountry, getCountries, searchCountry } from '../../../../api/Api';
...
function AllCountries() {
    ...
    const handleDelete = async () => {
        ...
        try {
            setLoading(true)
            const res = await deleteCountry(itemId);
    ...
    const searchData = async (pageNo) => {
        ...
        try {
            setLoading(true);
            const res = await searchCountry({ page: pageNo, val: inpVal });
            setLoading(false);
            setListData(res?.data);
            setTotalCount(res?.totalCount);
                        ...
                        <table className="table aiz-table mb-0 footable footable-1 breakpoint-xl" style={{}}>
                            <thead>
                                <tr className="footable-header">
                                    <th className="footable-first-visible" style={{ display: 'table-cell' }}>#</th><th style={{ display: 'table-cell' }}>Name</th><th style={{ display: 'table-cell' }}>Code</th>
                                    <th className="footable-last-visible" style={{ display: 'table-cell' }}>Show/Hide</th>
                                    ...
                                    return <tr key={i}>
                                        ...
                                        <td className="footable-last-visible" style={{ display: 'table-cell' }}>
                                            <label className="aiz-switch aiz-switch-success mb-0">
                                                <input onchange={() => handleToggleChange(item?.is_active)} type="checkbox" checked={item?.is_active} />
