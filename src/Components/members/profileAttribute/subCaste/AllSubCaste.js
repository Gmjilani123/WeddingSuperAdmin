...
import { deleteSubCaste, getSubCastes, searchSubCaste } from '../../../../api/Api';
...
function AllSubCaste() {
    ...
    const getData = async (page) => {
        if (searchStatus) {
            ...
            try {
                setLoading(true);
                const res = await getSubCastes(page);
    ...
    const handleDelete = async () => {
        ...
        try {
            setLoading(true)
            const res = await deleteSubCaste(itemId);
    ...
    const searchData = async (pageNo) => {
        ...
        try {
            setLoading(true);
            const res = await searchSubCaste({ page: pageNo, val: inpVal });
            console.log('ListApiResponse---', res)
            setLoading(false);
            setListData(res?.data);
            setTotalCount(res?.totalCount);
                        ...
                        <table className="table aiz-table mb-0 footable footable-1 breakpoint-xl" style={{}}>
                            <thead>
                                <tr className="footable-header">
                                    <th className="footable-first-visible" style={{ display: 'table-cell' }}>#</th><th style={{ display: 'table-cell' }}>Sub Caste</th>
                                    <th data-breakpoints="md" style={{ display: 'table-cell' }}>Caste</th>
                                    <th data-breakpoints="md" style={{ display: 'table-cell' }}>Religion</th>
                                    <th className="text-right footable-last-visible" width="20%" style={{ display: 'table-cell' }}>Options</th></tr>
                            </thead>
                            <tbody>
                                {listData && listData?.map((item, i) => {
                                    return <tr key={i}>
                                        ...
                                        <td style={{ display: 'table-cell' }}>{item?.caste_id}</td>
                                        <td style={{ display: 'table-cell' }}>{item?.religion_id}</td>
