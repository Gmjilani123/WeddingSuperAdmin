...
import { deleteFamilyValue, deleteMaritalStatus, deleteOnBehalf, getFamilyValues, getMaritalStatuses, getOnBehalfs, searchMaritalStatus } from '../../../../api/Api';


function AllMaritalStatuses() {
    ...
    const getData = async (page) => {
        if (searchStatus) {
            ...
            try {
                setLoading(true);
                const res = await getMaritalStatuses(page);
    ...
    const handleDelete = async () => {
        ...
        try {
            setLoading(true)
            const res = await deleteMaritalStatus(itemId);
            setLoading(false)
    ...
    const searchData = async (pageNo) => {
        try {
            setLoading(true);
            const res = await searchMaritalStatus({ page: pageNo, val: inpVal });
            console.log('ListApiResponse---', res)
            setLoading(false);
            setListData(res?.data);
            setTotalCount(res?.totalCount);
                ...
                <div className="card">
                    <div className="card-header row gutters-5">
                        <div className="col-lg-4 text-center text-md-left">
                            <h5 className="mb-md-0 h6">All Marital Statuses</h5>
                        </div>
...
export default AllMaritalStatuses