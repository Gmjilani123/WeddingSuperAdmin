...
import { deleteFamilyValue, deleteOnBehalf, getFamilyValues, getOnBehalfs, searchFamilyValue } from '../../../../api/Api';
...
function AllFamilyValues() {
    ...
    const getData = async (page) => {
        if (searchStatus) {
            ...
            try {
                setLoading(true);
                const res = await getFamilyValues(page);
    ...
    const handleDelete = async () => {
        ...
        try {
            setLoading(true)
            const res = await deleteFamilyValue(itemId);
            setLoading(false)
    ...
    const searchData = async (pageNo) => {
        ...
        try {
            setLoading(true);
            const res = await searchFamilyValue({ page: pageNo, val: inpVal });
            console.log('ListApiResponse---', res)
            setLoading(false);
            setListData(res?.data);
            setTotalCount(res?.totalCount);
                ...
                <div className="card">
                    <div className="card-header row gutters-5">
                        <div className="col-lg-4 text-center text-md-left">
                            <h5 className="mb-md-0 h6">All Family Values</h5>
                        </div>
...
export default AllFamilyValues