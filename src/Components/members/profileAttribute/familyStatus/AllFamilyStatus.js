...
import { deleteFamilyStatus, deleteOnBehalf, getFamilyStatus, getOnBehalfs, searchFamilyStatus } from '../../../../api/Api';


function AllFamilyStatus() {
    ...
    const getData = async (page) => {
        if (searchStatus) {
            ...
            try {
                setLoading(true);
                const res = await getFamilyStatus(page);
    ...
    const handleDelete = async () => {
        ...
        try {
            setLoading(true)
            const res = await deleteFamilyStatus(itemId);
    ...
    const searchData = async (pageNo) => {
        ...
        try {
            setLoading(true);
            const res = await searchFamilyStatus({ page: pageNo, val: inpVal });
            console.log('ListApiResponse---', res)
            setLoading(false);
            setListData(res?.data);
            setTotalCount(res?.totalCount);
                ...
                <div className="card">
                    <div className="card-header row gutters-5">
                        <div className="col-lg-4 text-center text-md-left">
                            <h5 className="mb-md-0 h6">All Family Status</h5>
                        </div>
...
export default AllFamilyStatus