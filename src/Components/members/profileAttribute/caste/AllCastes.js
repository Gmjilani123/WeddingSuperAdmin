...
import { deleteCaste, deleteReligion, getCastes, searchCaste } from '../../../../api/Api';
...
function AllCastes() {
    ...
    const getData = async (page) => {
        if (searchStatus) {
            ...
            try {
                setLoading(true);
                const res = await getCastes(page);
    ...
    const handleDelete = async () => {
        ...
        try {
            setLoading(true)
            const res = await deleteCaste(itemId);
    ...
    const searchData = async (pageNo) => {
        ...
        try {
            setLoading(true);
            const res = await searchCaste({ page: pageNo, val: inpVal });
            setLoading(false);
            setListData(res?.data);
            setTotalCount(res?.totalCount);
                ...
                <div className="card">
                    <div className="card-header row gutters-5">
                        <div className="col-lg-4 text-center text-md-left">
                            <h5 className="mb-md-0 h6">All Castes</h5>
                        </div>
                            ...
                            <tbody>
                                {listData && listData?.map((item, i) => {
                                    return <tr>
                                        ...
                                        <td style={{ display: 'table-cell' }}>{item?.caste}</td>
