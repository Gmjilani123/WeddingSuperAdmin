...
import { deleteCity, getCities, searchCity } from '../../../../api/Api';
...
function AllCities() {
    ...
    const handleDelete = async () => {
        ...
        try {
            setLoading(true)
            const res = await deleteCity(itemId);
    ...
    const searchData = async (pageNo) => {
        ...
        try {
            setLoading(true);
            const res = await searchCity({ page: pageNo, val: inpVal });
            setLoading(false);
            setListData(res?.data);
            setTotalCount(res?.totalCount);
                        ...
                        <table className="table aiz-table mb-0 footable footable-1 breakpoint-xl" style={{}}>
                            <thead>
                                <tr className="footable-header">
                                    ...
                                    <th data-breakpoints="md" style={{ display: 'table-cell' }}>State</th>
                                    <th data-breakpoints="md" style={{ display: 'table-cell' }}>Country</th>
                            ...
                            <tbody>
                                {listData && listData?.map((item, i) => {
                                    return <tr key={i}>
                                        ...
                                        <td style={{ display: 'table-cell' }}>{item?.state_id}</td>
                                        <td style={{ display: 'table-cell' }}>{item?.country_id}</td>
