...
import { deleteMemberLanguage, getMemberLanguages, searchMembersLanguage } from '../../../../api/Api';
...
function AllMemberLanguage() {
    ...
    const getData = async (page) => {
        if (searchStatus) {
            ...
            try {
                setLoading(true);
                const res = await getMemberLanguages(page);
    ...
    const handleDelete = async () => {
        ...
        try {
            setLoading(true)
            const res = await deleteMemberLanguage(itemId);
    ...
    const searchData = async (pageNo) => {
        ...
        try {
            setLoading(true);
            const res = await searchMembersLanguage({ page: pageNo, val: inpVal });
            setLoading(false);
            setListData(res?.data);
            setTotalCount(res?.totalCount);
