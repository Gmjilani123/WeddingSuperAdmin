...
import { deleteLanguage, getAllLanguages, searchLanguage } from "../../../api/Api";
...
function Language() {
  ...
  const getData = async (page) => {
    if (searchStatus) {
      ...
      try {
        setLoading(true);
        const res = await getAllLanguages(page);
  ...
  const handleDelete = async () => {
    ...
    try {
      setLoading(true)
      const res = await deleteLanguage(itemId);
      setLoading(false)
  ...
  const searchData = async (pageNo) => {
    try {
      setLoading(true);
      const res = await searchLanguage({ page: pageNo, val: inpVal });
      setLoading(false);
      setListData(res?.data);
      setTotalCount(res?.totalCount);
        ...
        <div className="card">
          <div className="card-header row gutters-5">
            <div className="col-lg-4 text-center text-md-left">
              <h5 className="mb-md-0 h6">All Languages</h5>
            </div>
